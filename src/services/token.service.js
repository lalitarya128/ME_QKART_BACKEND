 ...
 */
 const generateAuthTokens = async (user) => {
  const expiresIn = Math.floor(Date.now()/1000) + config.jwt.accessExpirationMinutes * 60; // Convert minutes to seconds

  const token = generateToken(user.id, expiresIn, tokenTypes.ACCESS);
