...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  const existingProduct = cart.cartItems.find((p) => p.product.equals(productId));
...
const deleteProductFromCart = async (user, productId) => {
  ...
  cart.cartItems = cart.cartItems.filter((p) => !p.product.equals(productId));
  await cart.save();
...
const checkout = async (user) => {
  ...
  cart.cartItems.forEach((items)=>{
    price += items.product.cost * items.quantity
  })
  if(user.walletMoney < price){
    throw new ApiError(httpStatus.BAD_REQUEST,"Wallet balance is lower than cart price")
  }
  user.walletMoney = user.walletMoney - price;
  await user.save();
  cart.cartItems = [];
  await cart.save();
};
